...
class Pay {
		...
		public function __construct($driver, $config = array()) {
				...
				$this->config['notify_url'] = U("/payment/callback", array('apitype' => $apitype, 'method' => 'notify'), false, true);
				$this->config['return_url'] = U("/payment/callback", array('apitype' => $apitype, 'method' => 'return'), false, true);
			
				$config = array_merge($this->config, $config);

				/* 设置支付驱动 */
				$class = strpos($driver, '\\') ? $driver : 'Org\\Util\\Pay\\Driver\\' . ucfirst(strtolower($driver));
