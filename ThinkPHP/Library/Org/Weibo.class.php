...
class Weibo {
	...
	//get followers list
	const USER_FOLLOWERS_URL = "https://m.api.weibo.com/2/messages/subscribers/get.json";
	 ...
	 * 获取用户uid
	 */
	public function getNewFollowerUid() {
		$diff = array();
		$uid = '5645498038';
		$userInfo = $this->httpGet(self::USER_FOLLOWERS_URL . "?access_token=" . $this->token);
		// var_dump($userInfo);
		$result = json_decode($userInfo, true);
		$ids = $result['data']['uids'];

		while ($result['count'] == '10000') {
			$userInfo = $this->httpGet(self::USER_FOLLOWERS_URL . "?access_token=" . $this->token . "&next_uid=" . $result['data']['uids'][9999]);
			// var_dump($userInfo);
			$result = json_decode($userInfo, true);
			$tmp = $result['data']['uids'];
			$ids = array_merge($ids, $tmp);
		}
		// var_dump($ids);
		// var_dump(count($ids));
		// var_dump(S('weibo:ids'));

		if (S('weibo:ids') != NULL) {
			$last_ids = S('weibo:ids');
			var_dump(count($last_ids));
			$diff = array_diff($ids, $last_ids);
			var_dump($diff);
		}

		S('weibo:ids', $ids, 3600*24*30);
		// var_dump(S('weibo:ids'));

		return $diff;
	}

	//get user information by uid --by Yeqin_Zheng 2015.11.4
	public function getUserInfoByUid($uid) {
		$userInfo = $this->httpGet(self::USER_INFO_URL . "?access_token=" . $this->token . '&uid=' . $uid);
		return json_decode($userInfo, true);
	}

	//unfollow cache clear --by Yeqin_Zheng 2015.11.4
	public function unfollowCacheClear() {
		$uid = '5645498038';
		$userInfo = $this->httpGet(self::USER_FOLLOWERS_URL . "?access_token=" . $this->token);
		// var_dump($userInfo);
		$result = json_decode($userInfo, true);
