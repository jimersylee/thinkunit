...
class Upload {
    ...
    */
    public function setFileSecret($secret) {
        $this-> uploader -> setFileSecret($secret);
    }
    ...
    */
    public function deleteFile($path) {
        return $this-> uploader ->deleteFile($path);
    }

    /**
     * 文件转移
     * @param $pathArray 包含转移源路径（source）和转移目的路径（target）的一个二维数组，array（array（'source' => $source1, 'target' => $target1）,...）
    * @return array
     */
    public function shiftFile($pathArray) {
        return $this-> uploader ->shiftFile($pathArray);
    }
