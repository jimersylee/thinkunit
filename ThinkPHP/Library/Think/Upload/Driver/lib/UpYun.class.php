...
namespace Think\Upload\Driver\lib;
...
class UpYun {
	...
	* @return object
	*/
	public function __construct($bucketname, $username, $password, $endpoint = NULL, $timeout = 720) {
    ...
    public function writeFile($path, $file, $auto_mkdir = False, $opts = NULL) {
        ...
        if (isset($opts[self::X_GMKERL_THUMBNAIL]) || isset($opts[self::X_GMKERL_TYPE])) {
            #echo 222;
            if (!isset($opts[self::X_GMKERL_QUALITY])) $opts[self::X_GMKERL_QUALITY] = 70;
            if (!isset($opts[self::X_GMKERL_UNSHARP])) $opts[self::X_GMKERL_UNSHARP] = 'true';
        } else {
           # echo 444;
            if (!isset($opts[self::X_GMKERL_TYPE])) $opts[self::X_GMKERL_TYPE] = "fix_width_or_height";
            if (!isset($opts[self::X_GMKERL_VALUE])) $opts[self::X_GMKERL_VALUE] = "1440x960";
            if (!isset($opts[self::X_GMKERL_QUALITY])) $opts[self::X_GMKERL_QUALITY] = 70;
            if (!isset($opts[self::X_GMKERL_UNSHARP])) $opts[self::X_GMKERL_UNSHARP] = 'true';
        }

        if (!isset($opts[self::X_GMKERL_ROTATE])) $opts[self::X_GMKERL_ROTATE] = 'auto';
	...
	*/
    ...
    public function shiftFile($pathArray) {
        ...
        foreach($pathArray as $key => $vo) {
            $result[$key] =  $this->writeFile($vo['target'], $this->readFile($vo['source']), true);
